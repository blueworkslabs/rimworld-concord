...
if(scripted)throw Error('Use the separate scripted native runner');
if(![1,2,3].includes(config.run))throw Error('Exactly three predeclared run identities');
const policy='campfire-v1';
const protocol={policy,run:config.run,coreCalls:8,pawnCalls:12,cooldownTicks:60,windowTicks:36000,nativeMs:600000,pausedInference:config.run!==3,wallMs:1500000,jevCalls:0};
const coreBackend=new ClaudeDecisionBackend({ledgerPath:config.ledger+'.core',scratchRoot:config.scratchRoot+'/core',trial:'campfire-core-v1'});
const pawnBackend=new ClaudeDecisionBackend({ledgerPath:config.ledger+'.pawns',scratchRoot:config.scratchRoot+'/pawns',trial:'campfire-pawns-v1'});
...
const command=`env CONCORD_CAMPFIRE_RUN=${config.run} CONCORD_TRIAL_POLICY=${quote(policy)} CONCORD_TRIAL_ID=${quote(runId)} RIMWORLD_LAB_ROOT=${quote(config.labRoot)} bash ${quote(config.remoteRepo+'/scripts/run-campfire-live-lab.sh')} ${cold?'cold':'game'}${scripted?' --scripted':''}`;
...
async function handle(line){
 ...
 if(isDecision){if(!['decision','core','core-answer'].includes(m.mode)||(m.mode==='core'?++coreCount>8-Number(before.core.attempts):++pawnCount>12-Number(before.pawns.attempts)))throw Error('Decision limit');}
...
const timer=setTimeout(()=>{failed=true;child.kill();},1560000);
...
const result={at:new Date().toISOString(),policy,protocol,kind:cold?'campfire-cold':'campfire-live',runId,passed:!failed&&code===0&&receipt?.passed===true,
