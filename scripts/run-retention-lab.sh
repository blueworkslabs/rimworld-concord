...
case "${1:-}" in
 ...
 game) concord_entry=retention-game; concord_args=() ;;
 cold) concord_entry=retention-game; concord_args=(--cold) ;;
 *) exit 2 ;;
esac
if [[ "${2:-}" == --scripted ]]; then concord_args+=(--scripted); fi
exec flock -n "$RIMWORLD_LAB_ROOT/concord/coordinator.lock" env CONCORD_RETENTION_LOCKED=1 CONCORD_NEEDS_LOCKED=1 \
