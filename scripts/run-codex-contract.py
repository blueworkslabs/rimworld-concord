...
def config(catalog,provider='openai',url=None):
 c={'model':MODEL,'model_provider':provider,'approval_policy':'never','sandbox_mode':'read-only','project_doc_max_bytes':0,'include_environment_context':False,'web_search':'disabled','tools.experimental_request_user_input.enabled':False,'tools.update_plan.enabled':False,'tools.view_image':False,'orchestrator.skills.enabled':False,'orchestrator.mcp.enabled':False,'model_catalog_json':str(catalog),'model_reasoning_effort':'low'}
 ...
 c['model_providers.'+provider+'.request_max_retries']=0;c['model_providers.'+provider+'.stream_max_retries']=0
...
class Client:
 def __init__(self,root,catalog,provider='openai',url=None):
 ...
 def receive(self,deadline):
  x=self.q.get(timeout=max(.001,deadline-time.monotonic()))
 ...
 def run(self,case):
  ...
  while True:
   x=self.receive(deadline);self.events.append(x);p=x.get('params',{})
...
def main():
 ...
 try:
  ...
  if args.live:
   ...
   client=Client(root,catalog);client.initialize();client.account();receipt['auth']='native ChatGPT';save()
   for case in cases:
