...
const backend=scripted?{receipts:[],summary:()=>({attempts:0,reservedEquivalentUSD:0,estimatedUsageUSD:0}),close(){},async decide(){return {kind:'accept',reason:'Scripted dry-run consent'};},async reflect(){return {kind:'continue',reason:'Scripted native continuation'};}}:new ClaudeDecisionBackend({ledgerPath:config.ledger,scratchRoot:config.scratchRoot,trial:'work-v1'});
const budget=new TrialBudget(config.jevLedger,.024,12,'jev-work-v1');
...
const command=`env CONCORD_TRIAL_ID=${quote(runId)} RIMWORLD_LAB_ROOT=${quote(config.labRoot)} flock -n ${quote(config.labRoot+'/concord/coordinator.lock')} node ${quote(config.remoteRepo+'/dist/src/work-live.js')}${cold?' --cold':''}${scripted?' --scripted':''}`;
...
const result={at:new Date().toISOString(),kind:cold?'work-live-cold':scripted?'work-scripted':'work-live',runId,passed:!failed&&code===0&&receipt?.passed===true,
