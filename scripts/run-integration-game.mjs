...
const policy='integration-v1';
const protocol={policy,calls:12,observationMs:300000,laterAtMs:150000,jevCalls:0};
const backend=scripted?{receipts:[],summary:()=>({attempts:0,reservedEquivalentUSD:0}),close(){},async speak(view){
 return {choice:'say',text:view.exchange.turn==='opening'?'I would like to eat before doing more. Could you help with the wood?':'I can consider a short load; let me see the actual offer.'};
},async reflect(view){
 const m=view.character.messages?.find(m=>m.to===view.character.id);
 if(!m)return {kind:'continue',reason:'Scripted unchanged outlook'};
 return {kind:'revise_outlook',reason:'Scripted attributed interpretation',update:{expectedRevision:view.character.outlook?.revision??0,notes:[{kind:'stance',subject:m.from,text:'My colleague has spoken about the wood; I will decide separately.',messageIds:[m.id]}]}};
},async decide(view){
 const food=view.pawn.facts?.find(f=>f.key==='need'&&f.value==='Food')?.level;
 if(food<.5)return {kind:'refuse',reason:'Scripted preference for a meal first'};
 if(view.proposal.action.trips>1&&!view.proposal.parentId)return {kind:'counter',reason:'Scripted smaller load',action:{...view.proposal.action,trips:1}};
 return {kind:'accept',reason:'Scripted fresh consent'};
...
const command=`env CONCORD_TRIAL_POLICY=${quote(policy)} CONCORD_TRIAL_ID=${quote(runId)} RIMWORLD_LAB_ROOT=${quote(config.labRoot)} bash ${quote(config.remoteRepo+'/scripts/run-integration-lab.sh')} ${cold?'cold':'game'}${scripted?' --scripted':''}`;
...
async function handle(line){
 ...
 if(isDecision){if(!['decision','reflection','social'].includes(m.mode)||++decisionCount>maxDecisions)throw Error('Decision limit');}
...
const result={at:new Date().toISOString(),policy,protocol,kind:cold?'integration-cold':scripted?'integration-scripted':'integration-live',runId,passed:!failed&&code===0&&receipt?.passed===true,
