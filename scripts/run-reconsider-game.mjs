...
const relevance=process.argv.includes('--relevance'),intent=process.argv.includes('--intent'),alternative=process.argv.includes('--alternative'),goal=process.argv.includes('--goal');
if([relevance,intent,alternative,goal].filter(Boolean).length>1)throw Error('Select one trial policy');
const policy=goal?'goal-v1':alternative?'alternative-v1':intent?'intent-v1':relevance?'interruption-v1':'reconsider-v1';
