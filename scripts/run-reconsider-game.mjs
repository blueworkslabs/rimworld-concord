...
const relevance=process.argv.includes('--relevance'),intent=process.argv.includes('--intent'),alternative=process.argv.includes('--alternative');
if([relevance,intent,alternative].filter(Boolean).length>1)throw Error('Select one trial policy');
const policy=alternative?'alternative-v1':intent?'intent-v1':relevance?'interruption-v1':'reconsider-v1';
...
const budget=new TrialBudget(config.jevLedger,.008,4,alternative?'jev-alternative-v1':intent?'jev-intent-v1':relevance?'jev-interruption-v1':'jev-reconsider-v1');
