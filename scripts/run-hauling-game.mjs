...
const command=`env RIMWORLD_LAB_ROOT=${quote(config.labRoot)} flock -n ${quote(config.labRoot+'/concord/coordinator.lock')} node ${quote(config.remoteRepo+'/dist/src/hauling-live.js')}${cold?' --cold':''}`;
...
async function handle(line){
 ...
 if(cold||seen.has(m.id)||active.size)throw Error('Unexpected request');
 const isDecision=m.type==='decision-request';
 if(isDecision){if(!['decision','reflection'].includes(m.mode)||++decisionCount>6)throw Error('Decision limit');}
 else if(m.type!=='appraisal'||++appraisalCount>6)throw Error('Appraisal limit');
...
const result={at:new Date().toISOString(),kind:cold?'hauling-live-cold':'hauling-live',passed:!failed&&code===0&&receipt?.passed===true,
