...
const policy='needs-v1';
const backend=scripted?{receipts:[],summary:()=>({attempts:0,reservedEquivalentUSD:0}),close(){},async decide(view){
 const food=view.pawn.facts?.find(f=>f.key==='need'&&f.value==='Food')?.level;
 if(food<.5)return {kind:'refuse',reason:'Scripted preference to eat before optional work'};
 if(!view.proposal.parentId)return {kind:'counter',reason:'Scripted smaller scope',action:{...view.proposal.action,trips:1}};
 return {kind:'accept',reason:'Scripted consent to one trip'};
...
const command=`env CONCORD_TRIAL_POLICY=${quote(policy)} CONCORD_TRIAL_ID=${quote(runId)} RIMWORLD_LAB_ROOT=${quote(config.labRoot)} bash ${quote(config.remoteRepo+'/scripts/run-needs-lab.sh')} ${cold?'cold':'game'}${scripted?' --scripted':''}`;
...
async function handle(line){
 ...
 if(isDecision){if(m.mode!=='decision'||++decisionCount>maxDecisions)throw Error('Decision limit');}
 ...
 try{
  ...
  try {if(isDecision){const output=await backend.decide(m.view,controller.signal);
...
const result={at:new Date().toISOString(),policy,kind:cold?'needs-cold':scripted?'needs-scripted':'needs-live',runId,passed:!failed&&code===0&&receipt?.passed===true,
