...
for concord_flag in "$@"; do case "$concord_flag" in --arm=harness|--arm=ui|--task=T[0-9]*|--save=lab-*|--model=*|--reasoning=low|--reasoning=medium|--reasoning=high|--ui-server=/*|--prepare-only=true) ;; *) exit 2;; esac; done
