...
mechanics=len(sys.argv)==4 and sys.argv[3]=='--mechanics'
assert len(sys.argv)==3 or mechanics
...
for i,(pawn,meal) in enumerate(zip(pawns,meals)):
 for n in pawn.findall('needs/needs/li'):
  if n.findtext('def')=='Food':n.find('curLevel').text=str(.9 if mechanics else [.4,.65,.55][i])
 ...
 # Saves load health directly; omitting it leaves -1, not a newly made item's
 # full health. RawBerries inherits MaxHitPoints=60 from OrganicProductBase.
 for key,value in [('def','RawBerries'),('id',meal.findtext('id').replace('MealSurvivalPack','RawBerries')),('pos',meal.findtext('pos')),('health','60'),('stackCount','75'),('forbidden','False')]:E.SubElement(raw,key).text=value
with open(dst,'xb') as out:r.write(out,encoding='utf-8',xml_declaration=True)
print(json.dumps({'fixture':'campfire-mechanics-v2' if mechanics else 'campfire-v2','removedReadyMealStacks':len(meals),'rawBerries':225,'berryHealth':60,'pawns':[p.findtext('id') for p in pawns],'skillsAndTraits':'unchanged','nativeSelfCare':True,'ordinaryWorkPriorities':'disabled; agreement jobs only'}))
