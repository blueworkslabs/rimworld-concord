...
test('core launcher refuse an owned staging lock and direct entry points fail before transport',async()=>{
 ...
 try {
  ...
  for(const mode of ['game','cold'])for(const launcher of ['scripts/run-core-lab.sh','scripts/run-core-events-lab.sh']){
  ...
  for(const entry of ['core-game','core-events-game']){
   const result=spawnSync(process.execPath,['dist/trials/'+entry+'.js'],{env,encoding:'utf8',timeout:5000});
   assert.equal(result.status,1);assert.match(result.stderr,/Use scripts\/run-core(?:-events)?-lab.sh/);
