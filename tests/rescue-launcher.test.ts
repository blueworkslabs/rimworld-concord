...
test('rescue launchers refuse an owned staging lock and direct entry points fail before transport',async()=>{
 ...
 try {
  ...
  for(const mode of ['fixture','game','cold','interruptions','interruptions-cold','native-log','native-log-cold','outlook','outlook-cold']){
  ...
  for(const entry of ['rescue-fixture','rescue-acceptance','interruption-acceptance','native-log-acceptance','outlook-acceptance']){
