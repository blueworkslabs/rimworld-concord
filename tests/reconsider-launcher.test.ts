...
test('reconsideration launchers refuse an owned staging lock and direct entry points fail before transport',async()=>{
 const dir=await mkdtemp(join(tmpdir(),'concord-reconsider-lock-'));await mkdir(join(dir,'concord'));
 const env:NodeJS.ProcessEnv={...process.env,RIMWORLD_LAB_ROOT:dir};delete env.CONCORD_RECONSIDER_LOCKED;
 ...
 try {
  ...
  for(const mode of ['fixture','game','cold']){
   const result=spawnSync('bash',['scripts/run-reconsider-lab.sh',mode],{env,encoding:'utf8',timeout:5000});
   assert.equal(result.status,1);assert.equal(result.stdout,'');assert.equal(result.stderr,'');
  }
  for(const entry of ['reconsider-fixture','reconsider-live']){
   const result=spawnSync(process.execPath,['dist/src/'+entry+'.js'],{env,encoding:'utf8',timeout:5000});
   assert.equal(result.status,1);assert.match(result.stderr,/Use scripts\/run-reconsider-lab.sh/);
