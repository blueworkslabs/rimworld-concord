...
// Generic completed agreement: a rescue.
...
// Multi-step ordered work for the delivered-units/steps summary (formerly two ordered-haul trips).
const cookProposal:Proposal={id:'p',pawn:'A',action:{kind:'cook',thing:'berries',target:'fire',x:1,z:1,count:10,meals:2,maxTicks:600},reason:'Meals',status:'accepted',standing:{status:'completed',deadline:600,steps:['a','b']}};
...
test('summary distinguishes delivered units, completed steps and stopped intentions',()=>{
 const d=domain(cookProposal);d.outcomes.a={id:'a',actor:'A',status:'completed',reason:'Delivered',x:1,z:1,delivered:10};d.outcomes.b={id:'b',actor:'A',status:'failed',reason:'Partial',x:1,z:1,delivered:2};d.proposals.p!.standing!.status='stopped';
