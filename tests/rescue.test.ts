...
async function setup(){
 const data:GameState={world:'w',epoch:'e',ticks:10,loaded:true,paused:false,actions:[],pawns:['A','B','C'].map(id=>({
  id,name:id,x:1,z:1,job:'',health:1,rescueReady:true,workReady:false,casualties:{epoch:'e',tick:10,mapId:1,radius:12,observations:[],visibleSubjects:[{target:'C',downed:true,inBed:false}],visibleBeds:[{bed:'bed1',x:4,z:5,medical:true,occupied:false,prisoner:false,slave:false,colonyOwned:true,forbidden:false}]},facts:[{key:'private',value:id,level:1}],
...
test('loss of the specific offered patient/bed cancels a pending rescue even without a native event',async()=>{
 for(const changed of ['target','bed','map','ready','withdrawn']){
  ...
  if(changed==='target')own.casualties!.visibleSubjects![0]!.downed=false;
  if(changed==='bed')own.casualties!.visibleBeds![0]!.occupied=true;
...
test('unrelated rescue option changes do not invalidate this question; fresh final checks work without polling',async()=>{
 ...
 await assert.rejects(c.pawn('A').decide(p.id,{name:'target-recovered',async decide(){data.pawns[0]!.casualties!.visibleSubjects![0]!.downed=false;return {kind:'accept',reason:'Outdated'};}}));
...
test('reflection cannot accept a rescue whose grounding vanished while it thought',async()=>{
 ...
 const result=await c.attend('A',{name:'stale-grounding',async reflect(){ran=true;data.pawns[0]!.casualties!.visibleSubjects![0]!.downed=false;return {kind:'proposal',proposalId:p.id,decision:{kind:'accept',reason:'Old observation'}};}});
 assert(ran);assert.equal(result.status,'failed');assert.equal(c.inspect().characters.A!.attention!.last!.status,'failed');assert.equal(c.inspect().characters.A!.reflections?.length??0,0);assert.equal(requests.length,0);assert.equal(c.inspect().proposals[p.id]!.decision,undefined);store.close();
});

test('shortlist absence or stale contradictory evidence does not invalidate a known rescue offer',async()=>{
 for(const stale of [false,true]){
  const {c,data,store,requests}=await setup(),p=await c.core().propose('A',rescue,'Rescue');
  const result=await c.pawn('A').decide(p.id,{name:'limited-view',async decide(){
   const own=data.pawns[0]!;own.rescue!.options=[];own.casualties!.visibleSubjects=[];own.casualties!.visibleBeds=[];
   if(stale){own.casualties!.tick--;own.casualties!.visibleSubjects=[{target:'C',downed:false,inBed:true}];}
   await c.observe();return {kind:'accept',reason:'Previously observed offer, native dispatch still validates'};
  }});
  assert.equal(result.status,'accepted');assert.equal(requests.length,1);store.close();
 }
});
