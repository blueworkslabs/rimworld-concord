...
test('haul-zone intent is bounded',()=>{
  ...
  assert.ok(!HaulZone.safeParse({...ok,w:65,h:8}).success);
  assert.ok(!HaulZone.safeParse({...ok,thing:'Wood Log'}).success);
  assert.ok(HaulZone.safeParse({...ok,thing:'ComponentIndustrial',label:'east shed',zoneId:950,hold:'growing'}).success);
});
