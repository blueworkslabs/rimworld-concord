...
test('trial table preserves every historical policy and adds a distinct twelve-call evaluation',()=>{
 for(const [name,p] of Object.entries(decisionTrials)){
  const calls=name==='legacy'?3:['reliability-v1','needs-v1','social-v1'].includes(name)?4:['work-v1','observer-v1','paced-v1','perspective-v1','outlook-check-v1'].includes(name)?12:6;
