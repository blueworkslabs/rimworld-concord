...
for(const entry of ['harness-perceive','harness-t1-scripted'])test(entry+' launcher and direct entry fail before transport without exclusive ownership',async()=>{
 ...
 try{await once(owner.stdout,'data');
  const locked=spawnSync('bash',['scripts/run-'+entry+'.sh','--save=lab-unused'],{env,encoding:'utf8',timeout:5000});assert.equal(locked.status,1);assert.equal(locked.stdout,'');assert.equal(locked.stderr,'');
  const direct=spawnSync(process.execPath,['dist/trials/'+entry+'.js'],{env,encoding:'utf8',timeout:5000});assert.equal(direct.status,1);assert.match(direct.stderr,/Exclusive lab lock required/);
