...
export function selfCareFollowup(d:Domain){
 ...
 return cares.map(a=>{
  ...
  // Completion report link (Fable, after the live run): the core's next exchange with the same
  // pawn after its eating choice ("did you eat?" / "yes") is a report about this receipt, so its
  // topics may close when the receipt verifies the meal. Only that one exchange, and never one
  // that is itself another eating choice; later, unrelated questions stay unlinked.
  const at=questions.findIndex(x=>x.id===a.questionId);
  const report=at<0?undefined:questions.slice(at+1).find(x=>x.pawn===a.pawn);
  ...
  return {id:a.id,pawn:a.pawn,kind:'eat' as const,questionId:a.questionId,sourceIds,...(linked?{reportQuestionId:linked.id}:{}),
