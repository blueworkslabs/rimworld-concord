...
if(process.env.CONCORD_OBSERVER_LOCKED!=='1')throw Error('Use scripts/run-observer-lab.sh fixture to acquire the staging lock');
...
const second=initial.pawns.find(p=>p.id!==actor.id&&p.id!==target.id)!;assert(second);
const secondOrigin=offset(0,14);
const config={secondary:{pawn:second.id,origin:secondOrigin,source:offset(5,14),destination:offset(6,14)},actor:actor.id,target:target.id,origin,source:offset(5),destination:offset(6),patientCell:offset(14),beds:[offset(14,2)]};
const name='lab-concord-observe-fixture-'+Date.now();
...
const action=pawn.hauling?.options.find(o=>o.x===config.destination.x&&o.z===config.destination.z&&o.trips>=3);assert(action,'Three-trip option must be native-grounded');assert(actual.pawns.find(p=>p.id===second.id)?.hauling?.options.some(a=>a.trips>=3),'Independent second hauling lane');
await mkdir(root+'/.runtime',{recursive:true});await writeFile(root+'/.runtime/observer-fixture.json',JSON.stringify({name,...config,second:second.id,action}));
