...
if(!['reconsider-v1','interruption-v1','intent-v1'].includes(policy))throw Error('Unknown trial policy');
const prefix=policy==='intent-v1'?'intent-live':policy==='interruption-v1'?'interrupt-live':'reconsider-live';
...
async function offer(pawn:string,action:any,reason:string){
 const p=await c!.core().propose(pawn,action,reason);await negotiate(p.id,pawn);
...
try{
 if(cold){
   ...
   if(!rescueChecked&&pump.status().modelStarted>0&&pump.status().pending===0){
    ...
    if(eligible){
     shutdown.later=(async()=>{
      const rescue=(await c!.core().rescueOptions(pawn))?.options.find(a=>a.target===target);
      if(!rescue){offers.push({pawn,status:'withdrawn-but-no-current-rescue-option'});return;}
      await offer(pawn,rescue,'You ended your supply agreement. Would you carry this observed downed colonist to this specific medical bed? This is a new optional agreement, not treatment; you may refuse or counter.');
  ...
  receipt.observation={wallMs:Date.now()-began,ticks:finish.ticks-start.ticks,samples,pausedSamples,thoughtSamples,ticksDuringThought,attention:pump.results,decisions,reflections,appraisals,operatorStops,rescueOfferStarted:!!shutdown.later,connected,pump:pump.status()};
