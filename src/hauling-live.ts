...
try{
 if(cold){
  ...
  const db=root+'/.runtime/hauling-live-'+Date.now()+'.db';store=new Store(db);const c=new Coordinator(store,b);await c.open();
  ...
  await b.admin('run');const start=await b.state(),began=Date.now(),end=began+180000;let samples=0;
  ...
  while(Date.now()<end){
   ...
   if(decisions<6&&appraisals<6)await pump.poll();else await c.reconcile();
  ...
  receipt.pairedRestore=true;receipt.limitations='Three-minute authored fixture; initial offers while paused, then continuous native play. Scripted core, no authored personality overrides. Model errors/cancellations are evidence, not rerolled. Not a long-run character-quality evaluation.';
 }
 receipt.passed=true;
