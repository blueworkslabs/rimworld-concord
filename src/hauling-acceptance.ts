...
try {
 ...
 if(cold){
  ...
  const repeated=await b.move({id:accepted.actionId!,epoch:initial.epoch,actor:pawn.id,action,untilTick:accepted.standing!.deadline});assert.equal(repeated.id,accepted.actionId);
