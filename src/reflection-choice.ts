...
export function reflectionChoices(view:AttentionView){
 ...
 if(view.intention&&view.intention.id===view.character.intention&&view.intention.pawn===view.pawn.id&&(view.intention.action.kind==='haul'||view.intention.action.kind==='haul-zone')&&view.intention.standing?.status==='running'&&!(view.requests??[]).some(r=>r.agreementId===view.intention!.id)){
