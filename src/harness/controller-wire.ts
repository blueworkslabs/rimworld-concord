...
export type GameToHost=
  ...
  {type:'stop';runId:string}|{type:'receipt';runId:string;receipt:unknown}|{type:'setup-failed';runId:string;error:string};
export type HostToGame=
  {type:'launched';runId:string;at:number;version:string;catalogSha256:string;argsSha256:string;preflight:{findings:string[]}|null}|
...
export class LineChannel<In extends {type:string},Out> {
  private waiters:{match:(m:In)=>boolean;resolve:(m:In)=>void;reject:(e:Error)=>void}[]=[];private handlers:((m:In)=>void)[]=[];
  ...
  constructor(private input:NodeJS.ReadableStream,private output:NodeJS.WritableStream){
    ...
    rl.on('close',()=>{this.closed=true;for(const w of this.waiters.splice(0))w.reject(Error('Wire closed'));});
  }
  ...
  next(match:(m:In)=>boolean,timeoutMs:number){
    return new Promise<In>((resolve,reject)=>{if(this.closed)return reject(Error('Wire closed'));
      const t=setTimeout(()=>reject(Error('Wire timeout')),timeoutMs);this.waiters.push({match,resolve:m=>{clearTimeout(t);resolve(m);},reject:e=>{clearTimeout(t);reject(e);}});});
  }
}
