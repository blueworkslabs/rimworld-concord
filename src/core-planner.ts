...
export function coreView(d:Domain,g:GameState){
 ...
 for(const own of g.pawns.filter(p=>d.characters[p.id])){
  ...
  const options:Action[]=(d.nativeHaul?[]:[...(production?.options??[]),...(rescue?.options??[]),...(haul?.options??[])]).filter(a=>(!deferred||!!invitation(a))&&!proposals.some(p=>p.pawn===own.id&&(p.status==='refused'||p.status==='withdrawn'||p.standing?.status==='stopped')&&sameWork(a,p.action))).slice(0,6);
