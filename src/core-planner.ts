...
export type CoreState={revision:number;brief:{id:string;text:string};topics:CoreTopic[];questions:CoreQuestion[];turns:{id:string;status:'running'|'applied'|'failed';choice?:CoreChoice;proposalId?:string;questionId?:string}[]};
...
export function coreView(d:Domain,g:GameState){
 ...
 const opportunities:{id:string;pawn:string;action:Action;observedTick:number;supply?:{label:string;sourceCount:number;destinationFree:number}}[]=[];
 ...
 const messages=core.questions.flatMap(q=>q.messages).map(m=>({id:m.id,tick:m.tick,from:m.from,to:m.to,text:m.text}));
 const agreements=proposals.slice(-24).map(p=>({id:p.id,pawn:p.pawn,action:structuredClone(p.action),status:p.status,reason:p.reason,
  ...(p.decision?{reply:structuredClone(p.decision)}:{}),progress:agreementProgress(d,p,g.ticks,g.actions)}));
 ...
 const requests=Object.values(d.requests??{}).map(r=>({id:r.id,pawn:r.pawn,target:r.target,status:r.status,reason:r.reason}));
 ...
 return {world:d.world,epoch:d.epoch,branch:d.branch,revision:core.revision,tick:g.ticks,brief:{...core.brief},
  ...
  limits:'No construction, cooking, work-priority changes or direct pawn control. No private needs, memories or outlooks. Topic text is a planner interpretation, not verified completion. Only linked agreement outcomes establish work completion. Silence is not agreement.'};
}
