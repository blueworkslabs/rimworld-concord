...
export function coreView(d:Domain,g:GameState){
 ...
 return {world:d.world,epoch:d.epoch,branch:d.branch,revision:core.revision,tick:g.ticks,brief:{...core.brief},
  ...
  capabilities:[...(d.nativeHaul?['propose the listed shared stockpile haul to each able pawn separately; the quota is fixed after the first acceptance; progress and credit come from the game ledger']:[]),'propose listed hauling/rescue/campfire construction/simple meals','adopt counter with fresh consent',ongoing?'one optional question to an eligible pawn after material shared status or outcome change; replies alone do not renew eligibility':'one optional addressed question per pawn, at most three total','wait'],
