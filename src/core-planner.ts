...
export type CoreState={schedule?:import('./core-scheduler.js').CoreSchedule;
 ...
 silentWake?:{tick:number;causes:import('./core-scheduler.js').CoreWake[]};revision:number;brief:{id:string;text:string};topics:CoreTopic[];questions:CoreQuestion[];turns:{id:string;status:'running'|'applied'|'failed';choice?:CoreChoice;review?:number;proposalId?:string;questionId?:string;
 /** Pawns whose message to the core woke this turn: a wait after them is never silent. */
 heard?:string[]}[]};
export type CoreQuestionView={observedTick:number;pawn:Pawn;character:Character;question:{id:string;text:string;from:'core';reportSelfCare?:ReturnType<typeof selfCareFollowup>[number];requeued?:string}};
