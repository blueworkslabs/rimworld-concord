...
export function crewReport(d:Domain,tick:number,status:import('./shared-status.js').SharedStatus[]=[],thinking:string[]=[]):CrewReport {
 ...
 const coreWaiting=lastTurn?.choice?.action.kind==='wait'||core?.silentWake?`Core: ${heard.length?`heard ${heard.join(', ')}; `:''}${lastTurn?.review&&lastTurn.choice?.action.kind==='wait'&&!core?.silentWake?'reviewed; still ':''}waiting on ${firstOpen?clip(firstOpen.text,120):'new events'}`:undefined;
