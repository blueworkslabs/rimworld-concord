...
import {WORK_TRIAL,laterOfferEligible,workSummary} from './work-trial.js';
...
input.on('line',line=>{try{if(line.length>32000)throw Error();const m=JSON.parse(line);if(m.type==='decision-result'){pendingThoughts.delete(m.id);decision.receive(m);}else appraisal.receive(m);}catch{connected=false;decision.close();appraisal.close();}});
input.on('close',()=>{connected=false;decision.close();appraisal.close();});
...
async function negotiate(id:string,pawn:string){
 if(decisions>=WORK_TRIAL.decisions){offers.push({id,pawn,status:'allowance-ended'});return;}
 ...
 catch{offers.push({id,pawn,status:'unavailable-no-retry',elapsedMs:Date.now()-began});}
 await record();
}
async function offerRound(pawns:string[],later=false){
 for(const pawn of pawns){
  if(!connected||decisions>=WORK_TRIAL.decisions)break;
  ...
  if(p.status==='countered'&&decisions<WORK_TRIAL.decisions){
...
try{
 if(cold){
  ...
  const initial=await b.state(),pawns=initial.pawns.map(p=>p.id);assert.equal(pawns.length,3);receipt.initial=pawns.map(id=>({pawn:id,hauling:initial.pawns.find(p=>p.id===id)!.hauling,facts:initial.pawns.find(p=>p.id===id)!.facts?.filter(f=>f.key.startsWith('need:'))}));
  ...
  let samples=0,pausedSamples=0,thoughtSamples=0,ticksDuringThought=0,priorTick=start.ticks,wasThinking=false,later:Promise<void>|undefined,laterDone=false;
  ...
  while(Date.now()-began<duration&&connected){
   await c.advanceIntentions();
   if(!later&&Date.now()-began>=second&&pump.status().pending===0){
    later=offerRound(pawns,true).catch(e=>{receipt.laterError=String(e);}).finally(()=>{laterDone=true;});
   }
   // Never overlap explicit negotiations with the single decision channel.
   if((!later||laterDone)&&decisions<WORK_TRIAL.decisions&&reflections<WORK_TRIAL.reflections&&appraisals<WORK_TRIAL.appraisals)await pump.poll();
  ...
  await pump.stop();
  // Close channels at the wall-clock boundary; pending attempts are retained, not replayed.
  decision.close();appraisal.close();if(later)await later;
  ...
  receipt.observation={wallMs:Date.now()-began,ticks:finish.ticks-start.ticks,samples,pausedSamples,thoughtSamples,ticksDuringThought,attention:pump.results,decisions,reflections,appraisals,operatorStops,laterRoundStarted:!!later,connected};
...
finally{await pump?.stop();try{await b.admin('pause');}catch{}decision.close();appraisal.close();input.close();await record();store?.close();await writeFile(root+'/.runtime/work-live-'+(cold?'cold':'game')+'.json',JSON.stringify(receipt,null,2));send({type:'receipt',receipt});}
