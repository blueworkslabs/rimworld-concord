...
function proposalChoices(p:Proposal){return [
 ...
 {choice:'defer',effect:'Not now: retire this offer without work or future consent. The core records your reply and will not automatically repeat this work. You may later deliberately request one fresh offer via reflection; that request is not consent.'},
...
export function modelPrompt(mode:'decision'|'reflection',view:Perspective|AttentionView){
 ...
 if(pawn.rescue)kinds.add('rescue');if(pawn.movement)kinds.add('move');
 ...
 if(pawn.production)contracts.production='Build: one campfire at the exact site using 20 wood from the named stack. Cooking is a separate agreement: exact campfire and raw-food stack, count ingredients per meal, at most meals simple meals. Native work, fuel, skills, needs and reservations apply. Neither success nor eating is guaranteed. Withdrawal or failure stops remaining work; partially built frames/materials remain. No automatic retry.';
 contracts.sharedStatus='Food/Rest bands about linked crew are explicitly shared telemetry, not visual observations or private thoughts. Read timestamp and fresh; unknown is not satisfied, and low is not a diagnosis or prediction.';
 ...
 if(kinds.has('haul-zone'))contracts.stockpileHaul={scope:'A shared colony stockpile (named by label), filled with the named item up to quota units in total by whoever hauls there. Accepting means you will haul there through your ordinary work, between meals and rest as usual. Others may help; each carrier is credited for their own deliveries.',consent:'Refusing, deferring or withdrawing keeps you out of this stockpile; a trip you are already carrying finishes and is recorded as started before withdrawing. Hauling wood to any other storage is not part of this agreement.',counters:'A different quota can be adopted only before anyone accepts; after that a counter is recorded but the quota stays.',execution:'The game decides trips, order and timing. No Food/Rest stop: the intent ends at the quota, expiry, withdrawal or an operator stop.'};
