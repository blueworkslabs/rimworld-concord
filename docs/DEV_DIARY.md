...
5. Place the reviewed entry in `diary/entries/SLUG.json`. Optional screenshots go under the diary's local image assets, with accurate alt text and a caption identifying scripted or live tests. Inspect images for private UI content. Do not publish game binaries, saves or databases.
...
The diary template and first Kimi-authored entry are being prepared as a team contribution. This document describes the intended workflow, not evidence of a deployed site. Final build/deployment settings and entry status are recorded in `diary/README.md` when the template is integrated.
