...
# Override the list to re-export only changed scenes: SCENES="CoreWakes" ./export.sh
set -euo pipefail
cd "$(dirname "$0")"
src=media/videos/concord_scenes/720p30
out=../media
mkdir -p "$out"
for s in ${SCENES:-ConsentLoop WhoKnowsWhat TimelineGuard CoreWakes NativeIntent}; do
