...
import {campfireInferencePassed,campfireDecisionMs} from './campfire-policy.js';
import {retainedDomain} from './retention-policy.js';
if(process.env.CONCORD_CAMPFIRE_LIVE_LOCKED!=='1')throw Error('Use scripts/run-campfire-live-lab.sh game|cold');
const root=new URL('../..',import.meta.url).pathname,cold=process.argv.includes('--cold');
const run=Number(process.env.CONCORD_CAMPFIRE_RUN);if(![1,2,3].includes(run))throw Error('Invalid trial index');
const pausedInference=run!==3;let operationDeadline=Date.now()+1500000;const b=new LabBridge(undefined,()=>operationDeadline);
const runId=process.env.CONCORD_TRIAL_ID;
if(!runId||!/^[0-9a-f-]{36}$/.test(runId)||process.env.CONCORD_TRIAL_POLICY!=='campfire-v1')throw Error('Trial identity required');
const receipt:any={passed:false,runId,policy:'campfire-v1',mode:pausedInference?'paused':'continuous',run,views:[],rounds:[],samples:[]};
...
const channel=new DecisionChannel(raw=>{const m=raw as any;guard.check();if(m.type==='decision-request'){
 ...
 if(m.mode==='core'?++coreAttempts>8:++pawnAttempts>12)throw Error('Core trial cap');
...
try{
 if(cold){
  const saved=JSON.parse(await readFile(root+'/.runtime/campfire-live-'+run+'-latest.json','utf8'));assert.equal(saved.runId,runId);assert.equal(saved.mode,receipt.mode);receipt.verifiesFailedRun=saved.failed;
 ...
 receipt.coreAttempts=coreAttempts;receipt.pawnAttempts=pawnAttempts;input.close();s?.close();await writeFile(root+'/.runtime/campfire-live-'+run+'-'+(cold?'cold':'game')+'.json',JSON.stringify(receipt,null,2));send({type:'receipt',receipt});
}
