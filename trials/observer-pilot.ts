...
let terminated=false;const onSignal=()=>{terminated=true;};process.on('SIGINT',onSignal);process.on('SIGTERM',onSignal);
const check=()=>{if(terminated)throw Error('Operator terminated pilot');};
let store:Store|undefined,c:Coordinator|undefined,rec:ReturnType<typeof recordingProcess>|undefined;
...
try{
 if(mode==='cold'){
  ...
  out.clockTicksPerSecond=Number((await exec('getconf',['CLK_TCK'])).stdout.trim());out.pageSize=Number((await exec('getconf',['PAGESIZE'])).stdout.trim());
  ...
  for(const variant of ['baseline','capture']){
   ...
   await exec('xdotool',['mousemove','1000','120'],{env:{...process.env,DISPLAY:':91',XAUTHORITY:'/run/rimworld-lab-display/Xauthority'},timeout:5000});
   ...
   while(Date.now()-row.startUnixMs<75000){
    check();if(rec?.failure)throw rec.failure;
    const elapsed=Date.now()-row.startUnixMs;
    if(rec&&rec.finished&&elapsed<74000)throw Error('Recorder stopped before observation ended');
    ...
    await c.reconcile();const state=await b.state();
    if(rec?.child.pid&&!rec.finished){const status=await readFile('/proc/'+rec.child.pid+'/stat','utf8').catch(()=>null);if(status){const fields=status.slice(status.lastIndexOf(')')+2).split(' ');row.recorderLast={cpuTicks:Number(fields[11])+Number(fields[12]),rssPages:Number(fields[21]),unixMs:Date.now()};}}
    row.samples.push({elapsedMs:Date.now()-row.startUnixMs,unixMs:Date.now(),tick:state.ticks,paused:state.paused,epoch:state.epoch,jobs:state.pawns.map(p=>({id:p.id,job:p.job})),actions:state.actions});await delay(450);
   }
 ...
 process.removeListener('SIGINT',onSignal);process.removeListener('SIGTERM',onSignal);store?.close();await writeFile(path,JSON.stringify(out,null,2));console.log(JSON.stringify({run,mode,passed:out.passed,error:out.error}));
}
