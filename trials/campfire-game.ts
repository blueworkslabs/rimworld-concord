...
async function inventory(name:string){
 const code=`import json,sys,xml.etree.ElementTree as E; r=E.parse(sys.argv[1]); ts=r.findall('.//maps/li/things/thing'); count=lambda d:sum(int(t.findtext('stackCount') or '1') for t in ts if t.findtext('def')==d); print(json.dumps({'wood':count('WoodLog'),'meals':count('MealSimple'),'berries':count('RawBerries'),'campfires':count('Campfire'),'invalidBerryHealth':sum(1 for t in ts if t.findtext('def')=='RawBerries' and int(t.findtext('health') or '-1')<=0),'ownedBills':len(r.findall(".//li[@Class='Concord.ConcordBill']")),'unfinishedForbidden':sum(1 for t in ts if 'Campfire' in (t.findtext('def') or '') and t.findtext('def')!='Campfire' and t.findtext('forbidden')=='True')}))`;
...
try{
 ...
 if(cold){
  const saved=JSON.parse(await readFile(marker,'utf8'));s=new Store(saved.db);c=new Coordinator(s,b);await c.restore(saved.completed);retainedDomain(c.inspect(),saved.completedDomain);receipt.completedColdInventory=await inventory(saved.completed);assert.equal(receipt.completedColdInventory.berries,205);assert.equal(receipt.completedColdInventory.meals,2);await c.restore(saved.checkpoint);retainedDomain(c.inspect(),saved.domain);receipt.coldRestore=true;receipt.report=(await b.state()).crewLog;receipt.cases=saved.cases;
 }else{
  const f=JSON.parse(await readFile(root+'/.runtime/campfire-mechanics-fixture.json','utf8'));await b.load(f.name);await b.admin('pause');
  ...
  const before='lab-concord-fire-before-'+Date.now();await c.checkpoint(before);receipt.initialInventory=await inventory(before);assert.equal(receipt.initialInventory.berries,225);assert.equal(receipt.initialInventory.meals,0);assert.equal(receipt.initialInventory.invalidBerryHealth,0);
  ...
  p=await offer('cook');await c.pawn(p.pawn).decide(p.id,decide('refuse'));assert.equal(Object.values(c.inspect().outcomes).filter(r=>r.kind==='cook').length,0);receipt.cases.push({kind:'cook-refusal',jobs:0});await c.restore(built);
  ...
  const between='lab-concord-fire-between-'+Date.now();await c.checkpoint(between);receipt.betweenInventory=await inventory(between);assert.equal(receipt.betweenInventory.ownedBills,0);assert.equal(receipt.betweenInventory.berries,215);assert.equal(receipt.betweenInventory.meals,1);const betweenDomain=c.inspect();await c.restore(between);retainedDomain(c.inspect(),betweenDomain);
  ...
  const completed='lab-concord-fire-completed-'+Date.now();await c.checkpoint(completed);receipt.completedInventory=await inventory(completed);assert.equal(receipt.completedInventory.berries,205);assert.equal(receipt.completedInventory.meals,2);assert.equal(receipt.completedInventory.ownedBills,0);const completedDomain=c.inspect();
  ...
  await writeFile(marker,JSON.stringify({db,checkpoint,domain,completed,completedDomain,cases:receipt.cases}));
 }
