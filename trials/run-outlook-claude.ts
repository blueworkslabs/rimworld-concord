...
import {bankVersion,preparedOutlookCases} from './outlook-cases.js';
export async function runOutlookClaude(file:string,output:string,create=(dir:string)=>new ClaudeDecisionBackend({ledgerPath:join(dir,'allowance.db'),scratchRoot:join(dir,'scratch'),trial:'outlook-check-v1'})){
 const data=readFileSync(file);const suite=JSON.parse(data.toString());
 const cases=preparedOutlookCases();
...
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href){
 if(process.argv.length!==5||process.argv[4]!=='--live')throw Error('Usage: run-outlook-claude CASES.json OUTPUT --live (fresh finite operator authorization required)');
 await runOutlookClaude(process.argv[2]!,process.argv[3]!);
}
