...
try{
 if(cold){
  ...
  if(scripted&&['eating-followup-v1','closure-followup-v1'].includes(policy)){const care=Object.values(c.inspect().selfCare??{});assert.equal(care.length,1);const result=c.inspect().outcomes[care[0]!.id];assert.equal(result?.status,'completed');assert((result?.delivered??0)>0,'scripted choice must consume food before cleanup');assert.equal(receipt.beforeCleanup.outcomes[care[0]!.id]?.status,'completed');}
  if(scripted&&policy==='closure-followup-v1'){const care=Object.values(c.inspect().selfCare!)[0]!;assert.equal(c.inspect().coreState!.topics.find(t=>t.sourceId===care.id)?.status,'resolved');assert.equal(c.inspect().coreState!.topics.find(t=>t.sourceId==='brief')?.status,'open');}
