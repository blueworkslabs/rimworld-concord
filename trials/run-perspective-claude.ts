...
import {bankVersion,preparedPerspectiveCases} from './perspective-cases.js';
export async function runPerspectiveClaude(file:string,output:string,create=(dir:string)=>new ClaudeDecisionBackend({ledgerPath:join(dir,'allowance.db'),scratchRoot:join(dir,'scratch'),trial:'perspective-v1'})){
 const data=readFileSync(file);const suite=JSON.parse(data.toString());
 const cases=preparedPerspectiveCases();
 ...
 try{
  ...
  for(const c of cases){
   receipt.attempts++;const row:any={id:c.id,status:'started'};receipt.results.push(row);save();const began=Date.now(),receiptIndex=backend.receipts.length,responseIndex=backend.rawResponses.length,failureIndex=backend.failures.length;
   try{
...
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href){
 if(process.argv.length!==5||process.argv[4]!=='--live')throw Error('Usage: run-perspective-claude CASES.json OUTPUT --live (fresh finite operator authorization required)');
 await runPerspectiveClaude(process.argv[2]!,process.argv[3]!);
}
