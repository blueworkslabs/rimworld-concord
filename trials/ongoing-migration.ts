...
export function assertMigrationMenu(v:CoreView,entries:Entry[],eligible:string[]){
 assert(!v.opportunities.some(o=>(o.action.kind as string)==='haul'),'Ordered hauling must be absent');
...
export function migrationCoverage(d:Domain,entries:Entry[],eligible:string[]){
 ...
 // Stores from before the retirement may still hold ordered-haul records.
 assert(!Object.values(d.proposals).some(p=>(p.action.kind as string)==='haul'),'Ordered hauling was proposed');
