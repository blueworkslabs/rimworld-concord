...
// Claim this mode's artifact before any game access. Reusing a run cannot erase evidence.
const receiptPath=prefix+'-'+mode+'.json';
await writeFile(receiptPath,JSON.stringify({run,mode,status:'started'}),{flag:'wx'});
...
try{
 if(mode==='cold'){
  ...
  for(const variant of ['near','far']){
   ...
   receipt.cases.push({variant,fixture,view:v});assert.equal(sight.status,'observed');assert(sight.status==='observed');assert.equal(sight.items.some(x=>x.thing===fixture.thing),variant==='near');assert.equal(sight.campfires.length,0);assert.equal(v.opportunities.length,0);
   ...
   if(variant==='far')farName=name;
  }
  // Observe an existing scripted completed checkpoint; do not repeat its jobs.
  const previous=JSON.parse(await readFile(root+'/.runtime/campfire-scripted-latest.json','utf8'));
  s?.close();s=undefined;c=undefined;const copied=prefix+'-old-checkpoint-copy.db';await writeFile(copied,await readFile(previous.db),{flag:'wx'});s=new Store(copied);c=new Coordinator(s,b);await c.restore(previous.completed);const built=await view();assert(built.foodSightings!.some(x=>x.status==='observed'&&x.campfires.length>0));receipt.cases.push({variant:'previously-built-checkpoint',view:built});
 ...
 await writeFile(receiptPath,JSON.stringify(receipt,null,2));console.log(JSON.stringify({passed:receipt.passed,error:receipt.error,run,mode}));
}
